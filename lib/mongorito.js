...
Model.prototype.inc = function (props, options) {
	...
	const updateProps = { $inc: props };

	Object.keys(props).forEach(prop => delete attrs[prop]);

	if (Object.keys(attrs).length > 0) {
		updateProps.$set = attrs;
	}
...
/**
 * Atomically modifies a model if the criteria evaluates to true.
 * Note! This operation only works for saved models
 *
 * @param {Object} criteria - query object to find the model
 * @param {Object} update - replacement object
 * @param {Object} options - options for update operation
 * @api public
 */

Model.prototype.modify = function (criteria, updates, options) {
	let self = this;

	assign(criteria, { _id: this.get('_id') });
	...
	return this._collection()
		...
		.then(function (collection) {
			...
			.then(res => {
				if (res.value === null) {  	// null is returned if query cannot find a target
					console.log('findAndModify fail ' + JSON.stringify(res, null, 2));
					// return Promise.reject('no model found with the given conditions');
					return Promise.reject(self);
				}
				console.log('findAndModify success ' + JSON.stringify(res, null, 2));
				self.set(res.value);
				return self._runHooks('after', 'update', options)
					.return(self);
			});
